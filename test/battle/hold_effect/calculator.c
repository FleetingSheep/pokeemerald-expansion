...
ASSUMPTIONS
{
    ASSUME(gItemsInfo[ITEM_CALCULATOR].holdEffect == HOLD_EFFECT_CALCULATOR);
}

SINGLE_BATTLE_TEST("Calculator correctly changes stats", s16 damage)
{
    u32 species;
    u32 item;

    PARAMETRIZE { species = SPECIES_MACHAMP>; item = ITEM_NONE; }
    PARAMETRIZE { species = SPECIES_MACHAMP; item = ITEM_CALCULATOR; }
    ...
    GIVEN {
        ...
        PLAYER(species) { Attack(200); Defense(100); Speed(100); SpAttack(100); SpDefense(100); Ability(ability); Item(item); }
...
{
    GIVEN {
        PLAYER(SPECIES_HAUNTER) { Attack(100); Defense(100); Speed(100); SpAttack(110); SpDefense(100); Ability(ABILITY_LEVITATE); Item(ITEM_CALCULATOR); }
